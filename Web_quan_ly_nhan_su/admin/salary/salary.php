                    ...
                    <tbody >
                        ...
                        $listcostomers_sql = "
                            SELECT
                                ...
                                p.ID_position,
                                CASE
                                    WHEN COUNT(t.ID_personnal) > 20 THEN 20
                                    ELSE COUNT(t.ID_personnal)
                                END AS count,
                                COALESCE(ps.personal_sales, 0) AS personal_sales,
                                CASE
                                    WHEN ss.ID_position IS NULL THEN 0
                                    ELSE COALESCE(ts.team_sales, 0)
                                END AS team_sales,
                                COALESCE(ps.personal_sales, 0) +
                                CASE
                                    WHEN ss.ID_position IS NULL THEN 0
                                    ELSE COALESCE(ts.team_sales, 0)
                                END AS total_sales,
                                ROUND(
                                    CASE
                                        WHEN COALESCE(ps.personal_sales, 0) < COALESCE(bs.target_1, 0) THEN COALESCE(ps.personal_sales, 0) * (COALESCE(bs.care_part_1, 0) / 100)
                                            WHEN COALESCE(ps.personal_sales, 0) BETWEEN COALESCE(bs.target_1, 0) AND COALESCE(bs.target_2, 0) THEN COALESCE(ps.personal_sales, 0) * (COALESCE(bs.care_part_2, 0) / 100)
                                            ELSE COALESCE(ps.personal_sales, 0) * (COALESCE(bs.care_part_3, 0) / 100)
                                    END,
                                    2
                                ) AS personal_salary,
                                ROUND(
                                    CASE
                                        WHEN COALESCE(ps.personal_sales, 0) +
                                            CASE
                                                WHEN ss.ID_position IS NULL THEN 0
                                                ELSE COALESCE(ts.team_sales, 0)
                                            END < COALESCE(ss.target_1, 0) THEN (COALESCE(ps.personal_sales, 0) +
                                                                                CASE
                                                                                    WHEN ss.ID_position IS NULL THEN 0
                                                                                    ELSE COALESCE(ts.team_sales, 0)
                                                                                END) * (COALESCE(ss.care_part_1, 0) / 100)
                                        WHEN COALESCE(ps.personal_sales, 0) +
                                            CASE
                                                WHEN ss.ID_position IS NULL THEN 0
                                                ELSE COALESCE(ts.team_sales, 0)
                                            END BETWEEN COALESCE(ss.target_1, 0) AND COALESCE(ss.target_2, 0) THEN (COALESCE(ps.personal_sales, 0) +
                                                                                                                CASE
                                                                                                                    WHEN ss.ID_position IS NULL THEN 0
                                                                                                                    ELSE COALESCE(ts.team_sales, 0)
                                                                                                                END) * (COALESCE(ss.care_part_2, 0) / 100)
                                        ELSE (COALESCE(ps.personal_sales, 0) +
                                            CASE
                                                WHEN ss.ID_position IS NULL THEN 0
                                                ELSE COALESCE(ts.team_sales, 0)
                                            END) * (COALESCE(ss.care_part_3, 0) / 100)
                                    END,
                                    2
                                ) AS team_salary,
                                ROUND(
                                ...
                                CASE
                                    WHEN (
                                        ...
                                        CASE
                                            ...
                                            ELSE COALESCE(ts.team_sales, 0)
                                        END
                                    ) >= psm.KPIs THEN 20 * psm.completion_time * 2
                                    ELSE (
                                        CASE
                                            WHEN COUNT(t.ID_personnal) > 20 THEN 20
                                            ELSE COUNT(t.ID_personnal)
                                        END * psm.completion_time
                                    )
                                END AS fixed_salary,
                                ts.ID_teams
                            FROM
                                personnal p
                            LEFT JOIN
                                timekeeping t ON p.ID_personnal = t.ID_personnal
                                AND (
                                    (t.check_in <= '08:00:00' AND t.check_out >= '11:00:00') OR
                                    (t.check_in <= '14:00:00' AND t.check_out >= '17:00:00') OR
                                    (t.check_in <= '18:00:00' AND t.check_out >= '21:00:00')
                                )
                                AND DATE_FORMAT(t.date, '%Y-%m') = '$formattedDate'
                            LEFT JOIN (
                                SELECT
                                    ID_persontion,
                                ...
                                FROM
                                    bill
                                WHERE
                                    DATE_FORMAT(Registration_Date, '%Y-%m') = '$formattedDate'
                                GROUP BY
                                    ID_persontion
                            ...
                            LEFT JOIN (
                                ...
                                WHERE
                                    DATE_FORMAT(b.Registration_Date, '%Y-%m') = '$formattedDate'
                                GROUP BY
                                    t.ID_teams
                            ...
                            LEFT JOIN
                                position psm ON p.ID_position = psm.ID
                            LEFT JOIN
                                base_salary bs ON p.ID_position=bs.ID_position 
                            GROUP BY
                                p.ID_personnal,
                                p.name,
                                p.ID_position
                        ...
                        while ($r = mysqli_fetch_assoc($result)) {
                            if (in_array($r["ID_personnal"], $team_ids) && $r["fixed_salary"]!=NULL) {
                                $amount1 = $r["personal_sales"];
                                $formattedAmount1 = formatCurrency($amount1);
                                $amount2 = $r["total_sales"];
                                $formattedAmount2 = formatCurrency($amount2);
                                $amount3 = $r["total_salary"];
                                $formattedAmount3 = formatCurrency($amount3);
                                $amount4 = $r["fixed_salary"];
                                $formattedAmount4 = formatCurrency($amount4);
                                $amount5 = $r["total_salary"] + $r["fixed_salary"];
                                $formattedAmount5 = formatCurrency($amount5);

                                if ($amount3 !== null) { // Kiểm tra nếu total_salary không phải là NULL
                                    ?>
                                    <tr>
                                        ...
                                        <td><?php echo $formattedAmount4; ?> VNĐ</td>
                                        <td><?php echo $formattedAmount3; ?> VNĐ</td>
                                        <td><?php echo $formattedAmount5; ?> VNĐ</td>
                                    </tr>
                                    <?php
                                }
                            }
