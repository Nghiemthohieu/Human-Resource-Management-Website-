        ...
        <div class="display-body">
            <div class="search">
                ...
                <div class="button-add">
                    <?php if(checkPrivilege('File.php')) { ?>
                        <button id="openModal" onclick="window.location.href='/Web_quan_ly_nhan_su/admin/timekeeping/downloadFile.php'">
            ...
            <div class="table">
                <table>
                    ...
                    <?php
                        ...
                        // Đếm tổng số bản ghi
                        $total_sql = "SELECT COUNT(*) FROM `timekeeping`";
                        $total_result = mysqli_query($conn, $total_sql);
                        $total_rows = mysqli_fetch_row($total_result)[0];
                        $total_pages = ceil($total_rows / $records_per_page);

                        // Lấy dữ liệu cho trang hiện tại
                        $lietke_sql = "SELECT * FROM `timekeeping` ORDER BY `ID` DESC LIMIT $start_from, $records_per_page";
            ...
            <div class="pagination">
                ...
                if ($current_page > 1) {
                    echo "<a href='timekeeping.php?page=1'>&laquo; First</a>";
                }

                // Hiển thị nút "Trang trước"
                if ($current_page > 1) {
                    $prev_page = $current_page - 1;
                    echo "<a href='timekeeping.php?page=$prev_page'>&lsaquo; Previous</a>";
                }
                ...
                for ($i = max(1, $current_page - $range); $i <= min($total_pages, $current_page + $range); $i++) {
                    ...
                    echo "<a $active href='timekeeping.php?page=$i'>$i</a>";
                }

                // Hiển thị nút "Trang kế tiếp"
                if ($current_page < $total_pages) {
                    $next_page = $current_page + 1;
                    echo "<a href='timekeeping.php?page=$next_page'>Next &rsaquo;</a>";
                }

                // Hiển thị nút "Trang cuối"
                if ($current_page < $total_pages) {
                    echo "<a href='timekeeping.php?page=$total_pages'>Last &raquo;</a>";
                }
                ?>
            </div>
        </div>
