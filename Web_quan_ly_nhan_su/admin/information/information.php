    ...
    <div class="display">
        ...
        <div class="display-body">
            <?php
                ...
                $personnal_sql = "SELECT personnal.*, position.name AS position_name, status.name AS status_name, teams.ID_teams AS ID_teams 
                                FROM personnal 
                                JOIN position ON personnal.ID_position = position.ID 
                                JOIN status ON personnal.ID_status = status.ID 
                                JOIN teams ON personnal.ID_personnal = teams.ID_personnal 
                                WHERE personnal.ID_personnal = '$ID_personnal'";

                $result = mysqli_query($conn, $personnal_sql);
                while ($r = mysqli_fetch_assoc($result)) {
                    // Mã hóa mật khẩu
                    $masked_password = str_repeat('*', strlen($r['password']));
            ?>
                    <p class="information">Mã key: <?php echo $r['ID_personnal']; ?></p>
                    <p class="information">Họ tên: <?php echo $r['Name']; ?></p>
                    <p class="information">Mã Cấp trên: <?php echo $r['ID_teams']; ?></p>
                    <p class="information">Ngày sinh: <?php echo $r['birthday']; ?></p>
                    <p class="information">SĐT: <?php echo $r['phonenumber']; ?></p>
                    <p class="information">Email: <?php echo $r['mail']; ?></p>
                    <p class="information">Văn phòng: <?php echo $r['office']; ?></p>
                    <p class="information">Thời gian bắt đầu công việc: <?php echo $r['startword']; ?></p>
                    <p class="information">Giới tính: <?php echo $r['gender']; ?></p>
                    <div class="flex">
                        <p class="information">Password: <?php echo $masked_password; ?></p>
                        <div class="button-add">
                            <button id="openModal" ><span>Đổi mật khẩu</span></button>
                        </div>
                    </div>
            <?php
                }
                        ...
                        <form id="changePasswordForm" action="change_password.php" method="post">
                            ...
                            <div class="flex-container">
                                <div class="flex-item1">
                                    ...
                                    <div class="error-message" id="currentPasswordError">Mật khẩu cũ không đúng.</div>
                                </div>
                            </div>
                            <div class="flex-container">
                                <div class="flex-item1">
                            ...
                            <div class="flex-container">
                                <div class="flex-item1">
                                    ...
                                    <div class="error-message" id="confirmPasswordError">Mật khẩu nhập lại không đúng.</div>
                                </div>
                            </div>
                            <div class="flex-container">
                                <div class="flex-item1">
                                    <input class="btn-password" type="submit" name="doimatkhau" value="Đổi mật khẩu">
                                </div>
                            </div>
                        </form>
                    </div>
...
<script>
   ...
   // Lấy thông báo lỗi
    ...
    //     if (new_password.value.trim() === confirm_password.value.trim() ) {
    ...
    document.getElementById("changePasswordForm").onsubmit = function(event) {
            ...
            if (isValid) {
                // Gửi yêu cầu AJAX để kiểm tra mật khẩu hiện tại
                let xhr = new XMLHttpRequest();
                xhr.open('POST', 'check_current_password.php', true);
                xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                xhr.onreadystatechange = function() {
